...
use App\Models\Masters\MstBkSearch;
...
class SearchItemHelper extends MstBkSearch {
    ...
    const RELEASE_SEARCHCONF = '/masters';
    const RELEASE_GROUP = [
        RS_STR_RENT => '/rents/group/',
        RS_STR_SALE => '/sales/group/',
    ];
    const RELEASE_RESULT = [
        RS_STR_RENT => '/rents',
        RS_STR_SALE => '/sales',
        self::RS_STR_NO_BUNDLE => '/rooms',
    ];
    ...
    public function getMapZahyouData($rentSaleStr) {
        ...
        $query = app('request')->all();
        if(DEMO_MODE) {
            $jsonData = self::_getDemoData($rentSaleStr, self::BK_DATA_MAP);
        } else {
    ...
    public function getMapAreaData($rentSaleStr) {
        if(DEMO_MODE) {
            $query = app('request')->all();
            $jsonData = self::_getDemoData($rentSaleStr, 'area');
            $siData = (isset($query['city_numbers'])) ? $query['city_numbers'] : null;
            if(!empty($siData)) {
                if(!is_array($siData)) {
                    $siData = explode(",", $siData);
                }
                $temp = [];
                foreach($siData as $si) {
                    if(isset($jsonData[$si])) {
                        $temp[] = $jsonData[$si];
                    }
                }
                $jsonData = $temp;
    ...
    public function getMapCityData($rentSaleStr) {
        $query = app('request')->all();
        if(DEMO_MODE) {
            $jsonData = self::_getDemoData($rentSaleStr, 'city');
        } else {
    ...
    public function getBkDetailData($rentSaleStr, $id) {
        $result = [];
        if(DEMO_MODE) {
            $result = (new JsonHelper())->getLocalJsonData(self::DEMO_RESULTJSON[$rentSaleStr] . "detail" . JSON_EXT, true);
        } else {
    ...
    public function getBkDetailTemplateData($rentSaleStr, $key, $buildType) {
        ...
        if(self::BK_TEMPLATE_DETAIL == $key) {
            $result = $this->_convertDetailData($result);
        }
        return $result;
    }

    private function _convertDetailData($data) {
        $result = [];
        $colsLimit = 3;
        $nowCols = 0;
        if(!empty($data)) { 
    ...
    public function getBkResultDataDirect($rentSaleStr, $query) {
        ...
        if(DEMO_MODE) {
            self::$resultData = (new JsonHelper())->getLocalJsonData(self::DEMO_RESULTJSON[$rentSaleStr] . "index" . JSON_EXT, true);
        } else {
            $param = [];
            foreach($query as $key => $value) {
                $value = self::_getAllImplode(",", $value);
    ...
    public function getBkSearchMaster($rentSaleStr, $key) {
        ...
        if(isset(self::BK_DATA_REALDATA[$key])) {
            if(empty(self::$searchData)) {
                if(DEMO_MODE) {
                    self::$searchData = (new JsonHelper())->getLocalJsonData("/Private/demo/SearchMaster/masters" . JSON_EXT, true);
                } else {
            ...
            if(empty(self::FROM_MASTER_DATA_NEW[$key])) {
                ...
                if (!empty(self::$searchData[$rentSaleStr][$rKey])) {
                    ...
                    if(DEMO_MODE) {
                        $result['search'] = (new JsonHelper())->getLocalJsonData(self::DEMO_SEARCHCONF[$rentSaleStr] . "/{$rKey}" . JSON_EXT, true);
                    } else {
