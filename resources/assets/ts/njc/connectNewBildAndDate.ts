 ...
 */
$(function () {
  var strNameNewBild: string = "new_building";
  var strNameBildDate: string = "building_year";
  ...
  if (0 < $radioNewBild.length && 0 < $radioBildDate.length) {
    $radioNewBild.on("change", function () {
      var radioval: string | number | string[] = $(this).val();
      var check: string = "";
      if ("1" == radioval) {
        check = radioval;
      }
      if (
        0 < $("label.btn-disabled").length ||
        (0 == $("label.btn-disabled").length && "1" == radioval)
      ) {
        ...
        $("input[name='" + strNameBildDate + "']")
          .parent()
          .removeClass("checked");
        $("input[name='" + strNameBildDate + "']:checked")
          .parent()
          .addClass("checked");
      }
    });
