...
function initBkDetailGoogleMap() {
  ...
  $("li.calcRoute").click(function () {
    var address =
      $(this).attr("data-ken") + " " + $(this).attr("data-eki") + "駅";
...
function addDomListener() {
  ...
  if (mapSearchContents.size() > 0) {
    ...
    var displayFlg =
      parseInt(bukken.zoom) <= dispSyuuhen ? "display:block;" : "display:none;";
  }
...
function loadingImg() {
  ...
  $("#" + divId)
    ...
    .css(
      "margin-top",
      (parseInt($("#map_canvas").css("height")) -
        parseInt($("#" + divId).css("height"))) /
        2 +
        "px"
    )
    .css(
      "margin-left",
      (parseInt($("#map_canvas").css("width")) -
        parseInt($("#" + divId).css("width"))) /
        2 +
        "px"
    )
