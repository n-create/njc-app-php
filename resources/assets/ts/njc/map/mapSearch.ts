...
(function ($) {
    ...
    function sizeChange () {
        ...
        if(isSmartPhone) {
            ...
            */
            ...
            var search_h:number = panel_h - $('.mapSideSearch .mapAddSearch').innerHeight() - $('.mapSideSearch .mapDiscription').innerHeight();
            $('#aspectwrapper').css('height', panel_h + 'px');
            $('.mapSideSearch .searchBox').css('height', search_h + 'px');
        }

        var con_div:any = $('.absolute').find('.divFacility');
        var tot_height:number = parseInt(con_div.css('height'));
    ...
    $.mapSearch.getAllBukken = function () {
        ...
        $.ajax({
            ...
            data: '&' + $('#' + formId).serialize() + "&sw_longitude=" + mapAreaSW.lng() + "&sw_latitude=" + mapAreaSW.lat() + "&ne_longitude=" + mapAreaNE.lng() + "&ne_latitude=" + mapAreaNE.lat() + "&map=1&limit=100",
    ...
    $.mapSearch.zahyouBukken = function (ido, keido) {
        ...
        $.ajax({
            type: "POST",
            url: mapSetting.urlZahyouBukken,
            data: 'latitude=' + ido + '&longitude=' + keido + '&map=1&' + $('#' + formId).serialize(),
            success: function (zahyouDat) {
                $.mapSearch.bukkenSliderBase();
                $("#" + bukkenDialogId).html(zahyouDat);
                $.mapSearch.bukkenSlider();
    ...
    $.mapSearch.__areaAjax = function () {
        $('.njcCityListAjax').click(function () {
            ...
            $.ajax({
                ...
                data: 'city_numbers[]=' + $(this).attr('label') + '&map=1&' + $('#' + formId).serialize(),
                success: function (dat) {
                    $('#' + dialogId).html('');
