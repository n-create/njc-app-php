...
(function ($) {
   ...
   */
  function checkSearchHide(flg: boolean) {
   ...
   * @return int 最小値以上の縦幅
   */
  function _getSpMinHeight(panel_h: number) {
    var min_h = 360;
  ...
  $.mapSearch = function (setting) {
    ...
    const mapSearch = mapSearchBase(mapSetting);
    ...
    if (mapSetting.ctrl === "groupingMap") {
      mapCenter = new google.maps.LatLng(
        mapSetting.bukkenRender[0].lat,
        mapSetting.bukkenRender[0].lng
      );
      mapSearch.preInit();
    } else {
      if (mapSetting.ido == "" && mapSetting.keido == "") {
        geocoder.geocode(
          { address: mapSetting.companyAddress },
    ...
    const mapSearch = {
      ...
      initMap: () => {
        ...
        if (mapSetting.companyIdo != "" || mapSetting.companyKeido != "") {
          var markerOpt = {
            ...
            position: new google.maps.LatLng(
              mapSetting.companyIdo,
              mapSetting.companyKeido
            ),
            map: map,
            clickable: false,
          };
        ...
        if ($("input[name='mapSearchContents']").size() > 0) {
          var displayFlg =
            parseInt(mapSetting.zoom) <= dispSyuuhen
              ? "display:block;"
              : "display:none;";
        }
          ...
          $.ajax({
            ...
            success: function (zahyouDat) {
              ...
              $.each(obj, function (countI, val) {
                var changeIcon = mapSearchGetBkIconRes(
                  mapSetting.chinOrBai,
                  val.building_type.key,
                  1
                );
                var markerOpt = {
                  icon: changeIcon,
      ...
      getGrpZahyou: () => {
        ...
        $.each(mapSetting.bukkenRender, function (countI, val) {
          var setIcon = mapSearchGetBkIconRes(
            mapSetting.chinOrBai,
            val.crui_no,
            1
          );
          ...
          google.maps.event.addListener(bukkenMarker, "click", function () {
            $.each(markers, function (i, val) {
              val.setIcon(
                mapSearchGetBkIconRes(mapSetting.chinOrBai, val.crui_no, 1)
              );
            });
            this.setIcon(
              mapSearchGetBkIconRes(mapSetting.chinOrBai, val.crui_no, 2)
            );
      ...
      zahyouBukken: (ido: number, keido: number) => {
        $("#map_canvas").__loading();
        $.ajax({
          type: "POST",
          url: mapSetting.urlZahyouBukken,
      ...
      zahyouGroupBukken: (ido: number, keido: number) => {
        ...
        $.ajax({
          ...
          url: mapSetting.urlZahyouBukken,
          data:
            "latitude=" + ido + "&longitude=" + keido + "&map=1&" + bukkenParam,
          success: function (zahyouDat) {
            mapSearch.bukkenSliderBase();
            $("#" + bukkenDialogId).html(zahyouDat);
            mapSearch.bukkenSlider();
      ...
      codeAddress: (areaname) => {
        if (geocoder) {
          geocoder.geocode(
            { componentRestrictions: { postalCode: areaname } },
            function (results, status) {
              if (status == google.maps.GeocoderStatus.OK) {
      ...
      bukkenSlider: () => {
        var elementConveyor: JQuery = $(".njcSliderConveyor");
        var elementItem: JQuery = $(".njcSliderItem");
        var elementViewer: JQuery = $(".njcSliderViewer");
        var elementClose: JQuery = $(".njcSliderClose");
        var elementAllBukken: JQuery = $(".njcSliderAllBukken");
        var elementBkCount: JQuery = $(".njcSliderBukkenCount");
      ...
      __getMarker: (targetContents, keyWord) => {
        var request = {
        ...
        placeService.search(request, function (results, status) {
          if (
            ...
            if (
              ...
              alert(
                "Googleから応答がありません、しばらくたってから再度お試しください。"
              );
            }
          }
          mapSearch.loadingRemove();
      ...
      codeAddressFreeword: () => {
        const searchMapMarker = new google.maps.Marker();
        ...
        service.textSearch(
          ...
          function (results, status) {
            if (status == google.maps.places.PlacesServiceStatus.OK) {
              ...
              if (alertMsgFlg === null) {
                $("div.mapAddSearch").after(
                  '<p id="mapAddSearchAlertId">対象の住所・施設が見つかりません。ご入力内容をご確認の上、再度ご入力ください。</p>'
                );
              }
            }
