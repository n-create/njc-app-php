  ...
  this.NJC.surroundingSlick = {
    ResStart: function (selector) {
      function _setCenterCboxDialog() {
        var center =
          $(document).width() / 2 -
          $("#colorbox.surroundings-dialog").width() / 2;
        $("#colorbox.surroundings-dialog")
          .clearQueue()
          .finish()
          .css({ left: center });
      }
      ...
      _slick.find(".surroundings-contents").on("click", function () {
        var num: any = $(this).data("number");
        var cb: any = $(".surroundings-pop-wrap > a");
        if (cb.length && cb[num]) {
          var colorConf = {
            open: true,
            maxWidth: "90%",
      ...
      _slick
        ...
        .find(".change-all-view-btn")
        .on("click", function () {
          if (_slick.hasClass("all-view")) {
            $(this)
              .removeClass("close-surroundings")
              .addClass("open-surroundings")
              .text("全て見る");
            _slick
              .slick(slickOption)
              .find(".slick-arrow")
              .css({
                height: _slick
                  .closest(".surroundings-list-wrap")
                  .outerHeight(true),
              });
            _slick.removeClass("all-view");
          } else {
            $(this)
              .removeClass("open-surroundings")
              .addClass("close-surroundings")
              .text("閉じる");
      ...
      _slick.find(".surrounding-image img").bind("load", function () {
        if (count == check++) {
          _slick
            ...
            .each(function (index) {
              var _that = $(this);
              var _thatImageHeight = _that
                .find(".surrounding-image")
                .outerHeight(true);
              var _thatMessageHeight = _that
                .find(".surrounding-message")
                .outerHeight(true);
              maxImageHeight =
                maxImageHeight < _thatImageHeight
                  ? _thatImageHeight
                  : maxImageHeight;
              maxMessageHeight =
                maxMessageHeight < _thatMessageHeight
                  ? _thatMessageHeight
                  : maxMessageHeight;
              if (index == count) {
                _that
                  .closest(".surroundings-list")
                  .find(".surrounding-image")
                  .css({ height: maxImageHeight });
                _that
                  .closest(".surroundings-list")
                  .find(".surrounding-message")
                  .css({ height: maxMessageHeight });
