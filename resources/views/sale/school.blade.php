...
<div class="container searchBox {{ RS_STR_SALE }}">
  <form id="bkSearchCheck" method="get" action="/{{ RS_STR_SALE }}/result" class="saleSearch school">
    <div class="searchIndex">
      ...
      <div class="searchBox">
        ...
        <div class="searchBoxInner {{ $key }}"><?php if($key == $searchManager::BK_DATA_SI_SYOGAKU || $key == $searchManager::BK_DATA_SI_CYUGAKU) { ?>
          <div id="head_{{ $key }}" class="h3 head_bar"><?= $name; ?><a href="#head_{{ $linkKey }}" class="manual-right">{{ $searchManager->getBkItemName(RS_STR_SALE, $linkKey) }}へ</a></div><?php } else { ?>
